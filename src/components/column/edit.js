	...
	return (
		<>
			<InspectorControls>
						...
						{attributes.mediaId != 0 &&
							<MediaUploadCheck>
								<MediaUpload
									...
									allowedTypes={['image']}
									render={({open}) => (
										<Button onClick={open} isSecondary>{__('Replace image', 'dpwpblocks')}</Button>
									)}
								/>
							</MediaUploadCheck>
						}
				...
				<PanelBody title={'Anchor (col ID)'} initialOpen={ false }>
					<InputControl
						label="Column Anchor (ID)"
						labelPosition="top"
						value={ colID }
						type="anchor"
						isPressEnterToChange
						onChange={ ( newStyle ) => setAttributes( {colID: newStyle} ) }
					/>
				</PanelBody>
			</InspectorControls>
			<i class="scap scap__top"> Start Col </i>
			<div { ...innerBlocksProps } />
			<i class="scap scap__bottom"> //End Col </i>
		</>
	);
}
